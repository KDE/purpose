    ...
    : KAbstractFileItemActionPlugin(parent)
{
    ...
    menu->setTitle(i18n("Share"));
    menu->setIcon(QIcon::fromTheme(QStringLiteral("document-share")));
    menu->model()->setPluginType(QStringLiteral("Export"));

    QObject::connect(menu, &Purpose::Menu::finished, menu, [menu](const QJsonObject &output, int error, const QString &errorMessage) {
