...
void AccountsModel::load()
{
    ...
    QDBusMessage msg =
        QDBusMessage::createMethodCall(u"org.kde.KOnlineAccounts"_s, u"/org/kde/KOnlineAccounts"_s, u"org.kde.KOnlineAccounts.Manager"_s, u"accounts"_s);
    QDBusReply<QList<QDBusObjectPath>> reply = QDBusConnection::sessionBus().call(msg);

    if (!reply.isValid()) {
        qWarning() << "error" << reply.error();
    }
    const auto accounts = reply.value();
...
{
    qWarning() << "slotAccountCreationFinished";
